...
These repository-level instructions apply to agents working with this source. Each
generated guardrail is embedded in every applicable distributed skill—universal
agent rules everywhere, scoped page/design rules only where declared—so marketplace
and plugin users inherit the rules governing their work when this root file is absent.
