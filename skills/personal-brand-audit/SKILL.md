...
description: Run a proof-backed inventory of a founder, executive, creator, or public figure and turn it into an exact 20-page visual audit plus an installable agent action plan. Use for personal-brand audits, credibility inventories, top-connection maps, reputation and AI/search visibility analysis, or a 30/60/90-day growth plan; use a local-business audit instead when the subject is only a company or GBP.
rule-scopes: published-html, design-review
---

# Personal Brand Audit

Produce the complete, inspectable answer to: **Who is this person publicly, what can a
stranger prove, what is unusually valuable, what is missing or contradictory, and which
agent-owned actions should happen next?**

This is an orchestrator, not a replacement for its specialist skills. The public audit
method is [How We Audit](https://blitzmetrics.com/how-we-audit/). The public installation
front door for the recommended agents is
[Install Local Service Spotlight](https://localservicespotlight.com/install/).
Do not substitute `/skills/`, a legacy ZIP page, or a model-specific local path for that
installation URL.

## Dependency preflight

A complete audit requires the eight routed skills named below. The `lss-everything` bundle
contains all eight. The smaller `authority-and-reputation` bundle contains this front door
and only the specialist lanes assigned to that topical bundle; it is suitable for intake and
a partial authority/reputation diagnostic, not the complete workflow.

Before starting, inventory which routed skills are actually available in the current account
or runtime and record the marketplace version. If any are missing:

- do not label the run a complete personal-brand audit;
- run only the safe installed lanes, label the artifact **PARTIAL**, and leave omitted lanes
  UNKNOWN or HOLD with the missing skill named;
- point the user to `https://localservicespotlight.com/install/` and name
  `lss-everything`, but do not claim it was installed until the named account proves that
  state; and
- do not simulate a missing specialist by silently weakening its acceptance contract.
...
2. **Exact 20-page visual PDF** — neither a short memo nor a padded appendix; build it from
   [references/20-page-pdf-blueprint.md](references/20-page-pdf-blueprint.md).
3. **Executive summary** — pages 1–2 of the PDF; page 1 is a self-sufficient visual snapshot
   for a reader who stops there, and page 2 explains five to eight non-obvious findings that
   change a decision.
4. **Agent action register** — every action names the owning function, routed skill, input,
   evidence IDs, approval gate, destination, due date, and observable acceptance test.
5. **Acceptance receipt** — run every applicable check in
   [references/acceptance-tests.md](references/acceptance-tests.md) and record failures,
   skipped checks, and missing access without turning them into passes.

Research notes alone are not the audit. A document source is not a rendered PDF. A rendered
PDF is not reviewed, delivered, received, or acted upon.

## Preflight: questions, scope, and authority

Read [references/intake-and-authority-boundary.md](references/intake-and-authority-boundary.md)
before research. Resolve the subject, desired buy box, languages and markets, owned domains,
company scope, requested delivery destination, private-source permissions, and any
credentialed systems.

Run two clearly labelled lanes:

- **Outside-in** is the default: anonymous public search, public pages, public metadata,
  public transcripts, and public reviews. It measures what a stranger or model can find.
- **Credentialed** uses analytics, social exports, CRM, CMS, advertising, email, or private
  files only when the source and read scope are explicitly authorized. It measures what the
  owner can see. It never silently replaces the outside-in view.

If credentialed access is absent, mark those metrics **UNKNOWN** and continue the public
audit. Do not equate “not visible publicly,” “not accessed,” and “does not exist.”

## Evidence language

Use these labels on every substantive claim, table row, chart, and recommendation:

- **OBSERVED** — the cited source directly displays the fact or claim. This means the
  source said it; it does not automatically make the underlying marketing claim true.
- **INFERRED** — a bounded synthesis supported by named evidence IDs. State the reasoning.
- **UNKNOWN** — not established by the sources or access checked. Name the question, owner,
  and next evidence needed. UNKNOWN is neither zero nor failure.

Keep source authority, evidence status, and freshness in separate fields. A first-party bio
can be directly observed but still be self-asserted. A stale independent page can be strong
historical evidence but weak current-role evidence.

## Identity and relationship controls

Before attributing anything to a common name, match at least **two independent attributes**:
employer or brand, exact handle, location, role chronology, co-founder, headshot, institution,
license, or another hard identifier. Record both attributes and the source for each. One
attribute is a hypothesis; a matching name is not identity.

Keep these two fields separate in the connection map:

- **Public association:** the two people appeared on the same episode, page, stage, event,
  company, or project, with a URL and date.
- **Relationship depth:** close, warm, current, endorsing, commercially connected, or able to
  make an introduction. This remains **UNKNOWN** unless the subject or another direct record
  confirms it.
...
lane while keeping identity resolution, contradiction adjudication, scoring, non-obvious
insight, and final synthesis with the strongest available judgment. Independently review the
finished artifact from a fresh context.

Then run the specialist sequence:

1. **`evidence-verification`** — resolve identity, namesakes, hard anchors, chronology,
   claim conflicts, and source quality. Nothing downstream may repair a wrong identity.
2. **`positive-mentions-harvester`** — collect appearances, testimonials, talks, reviews,
   client outcomes, press, and lighthouse proof; score and rank without padding the tail.
3. **`reputation-gap-analyzer`** — compare the verified proof against the desired buy box;
   separate missing proof from proof that exists but is buried or inconsistent.
4. **`geo-visibility-audit`** — when current model access is available, run known-item and
...
7. **`dollar-a-day-strategist`** — stage amplification only for proven organic winners.
   Live spend, audience changes, kill decisions, and scaling require the exact approved
   account, budget, dates, and metric.

Each specialist writes back to the same evidence IDs and action register. Do not paste eight
unreconciled reports into the PDF.

## Brand Authority exam

When a score is useful, apply the versioned anchors in
[references/brand-authority-rubric.md](references/brand-authority-rubric.md) and report the
Brand Authority exam as ten separately evidenced rows worth 0–10 each:

1. entity home and name-rank;
2. media relationships and offline authority;
3. content assets;
4. social proof;
5. monetizable positioning;
6. consistency and schema hygiene;
7. current-channel momentum;
8. link authority to the entity home;
9. search visibility beyond the name;
10. Knowledge Graph presence.

The headline is the arithmetic sum and **80/100 is the pass line**. A row receives a number
only after it has been checked. If any required row is UNKNOWN, show **INCOMPLETE — no pass
verdict**, the known subtotal, and the missing evidence; never enter zero merely because it
was not researched.

If the website is in scope, run `seo-audit` and the public
[personal-brand website QA](https://blitzmetrics.com/website-qa-audit/) as separate exams.
Do not average Brand Authority, SEO, Website QA, GEO, reputation, or social metrics into an
invented composite score.

## Analysis standard

Lead with evidence that changes action, not praise or an inventory dump. A useful audit
typically resolves:

- the exact positioning the market already corroborates versus the generic label the subject
  uses;
- authority that is stranded on employer, host, event, or review domains;
- name collisions, stale titles, conflicting biographies, broken canonical pages, and
  unsupported or inconsistent numbers;
- the strongest ten proof items and the buyer beliefs they support;
- public associations worth confirming, without inventing relationship depth;
- the content themes already supported by real recordings and lived experience;
- the company or partner opportunity that creates mutual value rather than extracting the
  person’s audience;
- the smallest 7-, 30-, 60-, and 90-day actions that move calls, qualified opportunities,
  revenue, or another named business outcome.
...
| Field | Requirement |
|---|---|
| Action ID | Stable identifier used in the PDF and receipt |
| Finding / evidence | Evidence IDs and the current OBSERVED / INFERRED / UNKNOWN state |
| Owning function | Research, content, web, analytics, client success, paid media, or another durable function |
| Routed skill | Exact skill that performs the work |
| Input / dependency | Source asset, access, decision, or predecessor action |
| Destination | Exact page, profile, document, campaign, or system |
...
- The public and credentialed lanes are separate, with coverage and capture dates.
- The Brand Authority exam is complete and sums correctly, or is explicitly INCOMPLETE; no
  unrelated scores are averaged.
- The PDF renders to exactly 20 pages, passes visual review at desktop and print size, and has
  a meaningful real image, screenshot, chart, or evidence-backed diagram on every page.
- Page 1 contains the decision-useful cover and visual executive snapshot; page 2 contains
  the deeper non-obvious summary; the top proof and top connections are ranked and caveated.
- The agent map links and QR code decode exactly to
  `https://localservicespotlight.com/install/`, and the anonymous destination resolves before
  delivery. The guide's repository, bundle, released version, and skill count match the
  released marketplace manifest; a stale HTTP 200 is a failure.
- Every 7/30/60/90-day action satisfies the action-plan contract and preserves the human gate.
- A fresh-context reviewer runs the acceptance suite. Failures and untested checks remain
  visible.
- The receipt distinguishes generated, rendered, reviewed, sent/published, delivered,
  received, installed, enabled, tested, scheduled, and observed. No state stands in for
  another.
...
<!-- shared-rule:lead-with-the-point-in-plain-words:start -->
## Lead with the point, in plain words

- **Write it so a fifth grader gets it, and put what matters most first, in bold.** This
  covers every surface a human reads: email, Basecamp comments, texts, briefs, run
  summaries, client updates.
- **The subject line is the outcome**, not the job name, and it says whether the reader
  has to do anything. `2 new members scored and live — nothing needs you` beats
  `Daily scoring — ops QA: 2026-09-07`.
- **The first line is one bold sentence**: what happened, then `Nothing needs you` or
  `Needs you: <the one thing>`. Then at most three bold bullets — money, risk, or a
  decision only the reader can make. Say what it means, not what ran.
- **Everything technical goes below a line that reads
  `Details (skip unless something looks wrong)`** — unchanged and complete. Above that
  line: no file names, exit codes, IDs, hashes, timestamps, cron strings or script names.
  Under 150 words, sentences under 15 words, fifth-grade reading level.
- **Nothing is deleted and nothing is softened.** Detail is not the problem, order is. A
  failure is the bold first line, not a footnote — a shorter top never means a cleaner
  story than the run actually had.
- This is `visuals-above-the-fold` applied to writing, and the sibling of
  `lead-with-a-visual-executive-summary`: put the thing that matters where the eye lands.
  A dense report is not a thorough report, it is an unread one.
<!-- shared-rule:lead-with-the-point-in-plain-words:end -->
