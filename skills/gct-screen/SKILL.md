---
name: gct-screen
description: Screen whether a business has clear Goals, Content, and Targeting plus independently checkable proof before it enters the Social Amplification Engine. Use for prospect qualification, SAE onboarding, or re-qualification after an offer, market, or capacity change.
...
references:
  ...
  - skills/positive-mentions-harvester
...
Use this skill to decide whether there is something true, focused, and repeatable
to amplify. It is a public-evidence diagnostic. It does not create an engagement,
add a prospect to the client roster, authorize work, or promise results.

Canonical public guide: https://blitzmetrics.com/social-amplification/

## Output states

Use exactly one state. Do not collapse missing evidence into a negative finding.
...
- `VERIFIED` — corroborated at the level required for the claim.
- `CONTRADICTED` — credible sources disagree.
- `EXPIRED` — evidence is too old for the decision.

`UNKNOWN` is not zero, false, clean, or a decline. Every unknown records the exact
question, why it is unavailable, the owner, the due/escalation date, and the action
...
- Exact legal/trading entity, canonical domain, owner, market, location, and service.
- Public website, Google Business Profile, current reviews, source content, and ads.
- Existing audit, brand brief, or private evidence only when access and consent allow.
- The intended service scope, capacity owner, and measurement systems.

Prospect mode is public-read-only. Do not add a prospect to `clients/ROSTER.md`,
Basecamp, a schedule, or a Git repository. Keep PII and private submissions in the
approved private intake system; the Git receipt contains only nonsecret aggregate
status and source-safe links.
...
2. **Goal** — one measurable business outcome and reporting period; not “awareness.”
3. **Target** — one bounded ideal customer/problem, geography, job size, and exclusion.
4. **Offer and technique** — one clear outcome and repeatable method. The swap test
   fails if the statement could sit unchanged on a competitor’s site.
5. **Proof** — real customers and outcomes: current reviews, named work, licenses,
   before/after evidence, or third-party corroboration appropriate to the claim.
6. **Source content** — real recordings, reviews, stories, or assets already exist to
   feed Produce → Process → Post → Promote. A plan to create proof is not proof.
7. **Execution capacity** — a named owner, delivery/response capacity, capture cadence,
   claim permissions, and willingness to supply evidence.
8. **Measurement readiness** — traffic → inquiry → qualified lead → booking/job →
   collected/recognized revenue are defined, with systems of record or explicit
   onboarding blockers. Missing connectors stay `UNKNOWN`; they are never zero.

Write both alignment sentences:

> I help **[specific X]** achieve **[measurable Y]** through **[distinct Z]**.

> We will pursue **[Y]** by showing **[named proof/content]** to **[X]**.

If either sentence is untrue or interchangeable with a competitor, the screen cannot
qualify.
...
|---|---|---|---|
| `NEW_IDEA` | No customers or delivered work yet | `DEVELOP` | Deliver the first real work and collect proof. |
| `NO_PROOF` | Research completed; no corroborated outcomes or customers found | `DEVELOP` | `positive-mentions-harvester` or proof capture. |
| `UNDIFFERENTIATED` | Swap test fails and no distinct technique is evidenced | `DEVELOP` | `business-brand-strategist`. |
| `TOO_BROAD_ICP` | One message tries to serve incompatible audiences/geographies/jobs | `DEVELOP` | Select one bounded ICP. |
| `TOO_MANY_OFFERS` | No primary repeatable offer/process can be selected | `DEVELOP` | Choose the one offer to amplify. |
| `UNCLEAR_GOAL` | No measurable outcome/reporting period | `DEVELOP` | Define the goal and measurement contract. |
| `CAPACITY_RISK` | Demand cannot be fulfilled or followed up | `DEVELOP` | Name capacity owner and remedy. |
...
1. Resolve identity first and record the two-attribute match.
...
6. For `QUALIFIED_PENDING_REVIEW`, have a second fresh evaluator review the evidence.
   Disagreement becomes `DISCOVERY_REQUIRED`; never average the verdicts.
7. Save the private assessment in the configured case system and a secrets-free receipt.
   Do not create an Active Client row or recurring job.
...
next_action: {owner_function: client_success, due: YYYY-MM-DD, acceptance_test: text}
```

Each evidence item includes `url_or_record`, `source_class`, `claim`, `checked_at`,
and `collector`. Never place private revenue, lead-level data, PII, or secrets in Git.

## Launch gates are separate

After fit review, the following sequence is mandatory:

`QUALIFIED_PENDING_REVIEW → accepted scope/agreement receipt → Ops roster decision → onboarding → stage-only execution`

- Only an approved engagement can create an `Active Client` row.
- A `Special Project` needs an explicit scope receipt and does not enter the recurring
  Active Client cohort automatically.
- `Not Active` always stops.
- Content may be drafted while plumbing is blocked, but no publish or amplification is
  allowed until Search Console, CMS, GA4, and Tag Manager gates are verified green.
- Qualification never authorizes publishing, messaging, permissions, or spend.
...
- One next action has an owner function, date, and acceptance test.
- No roster, client system, schedule, message, publish, permission, or spend mutation
  occurred from a prospect screen.

## Related

`evidence-verification` → `gct-screen` → agreement/roster gate →
`one-session-client-onboarding` → `social-amplification-engine`
