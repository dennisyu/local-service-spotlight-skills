...
field is DERIVED from the source MAA. See `example-ridgeline.json` for a complete,
valid GBP instance.
...
| Field | Type | Notes |
|---|---|---|
| `eyebrow` | string | Fixed: `"Maps Performance · Google Business Profile"` (GA4 pages use `"Website Performance · GA4"`). |
| `client` | string | Client display name (the big H1). |
| `subline` | string | One line: primary category · city, state · `location_id`. |
...
| `pills` | array[4] | KPI tiles, exactly four: primary action · secondary · third · Maps views (e.g. Calls · Website clicks · Directions · Maps views). See below. |
...
| `citations` | object | `{ "project_url", "gbp_url", "date" }`. `gbp_url` is the profile's Maps link from the roster (`maps_uri`). |
...
## Derivation mapping (MAA → spec) — GBP version is in SKILL.md Step 2; the GA4 table below is kept for parity
