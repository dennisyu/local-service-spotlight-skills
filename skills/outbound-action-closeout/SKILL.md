...
Make the acting agent—not Dennis—the owner of execution, proof, and closeout.

## Before acting

- Resolve the exact destination, audience, thread or record, and authorized action.
- Use the destination's canonical write path. A notification address is not a
  substitute for the source system.
- If routing, authority, or the intended audience is unclear, fail closed. Do
  not send, post, publish, or ask Dennis to relay it.

## Required closeout

1. Perform only the separately authorized action.
2. End the human-visible content with:
...
5. Leave the next action with its real owner. Do not make Dennis forward,
   restate, mirror, or babysit work the agent can complete.

For Gmail, after a verified reply or send, archive the exact completed thread.
Restore it after seven days only if no human reply arrived. Boomerang, delivery
notices, vacation responders, Fancy Hands system mail, and other automation do
not count as human replies. Preserve the live state when a human has replied.

## Unattended routines

A scheduled session may not load repository rules or prior conversation. Its
prompt must explicitly name this skill and carry these critical rails itself:
...
- closeout/archive behavior; and
- fail closed with an internal blocker when any assertion cannot be proved.

Do not use a routine Test run when it could contact a person, publish, purchase,
delete, or change production. Test with a safe draft or read-only canary.

## Completion receipt

Return the source-system record ID or URL, verified audience, read-back result,
next owner, and inbox/queue state. Preserve `UNKNOWN` for delivery, review, or
ownership that was not independently proved.
