---
name: ga4-client-view
description: "Render a finished GA4 website MAA into the client-facing one-page executive summary (print-ready PDF + self-contained HTML), following the Local Service Spotlight display standard: stoplight coloring (red = critical/missing), a \"prepared by\" human owner, the client logo, the two standard charts (13-week trend + channel breakdown), an owned \"what to do next\" checklist, a source-citations footer, and \"we\" voice throughout. It does NO new analysis: every value is DERIVED from the source MAA, never invented. Runs as the final step after ga4-website-maa writes the report. Trigger on \"render the GA4 client view\", \"make the GA4 report pretty\", \"display version of the GA4 MAA\", \"one-pager for {client}\"."
...
client-facing layer that sits on top of it:

1. A **self-contained HTML** page-1 executive one-pager (`{client}_page1_{date}.html`) —
   all CSS inline, images embedded as data URIs, so it renders anywhere.
2. A **print-ready PDF** of that page (`{client}_page1_{date}.pdf`), Letter size.
3. Optionally, a **merged full PDF** (`{client}_full_{date}.pdf`) = page 1 + the full
   MAA (pages 2 onward), when a MAA PDF is supplied.

The full MAA is preserved intact as the later pages. The goal is to solve "too
dense for the owner, and it doesn't look like our standard" by leading with a
30-second executive read built to the format Dennis teaches — without touching
the analysis underneath.

## What this skill does NOT do

- **No analysis.** It never diagnoses, re-weights, or second-guesses the MAA. If a
  finding is not in the MAA, it does not appear on page 1.
- **No edits to the source MAA.** The dated MAA is read-only here.
- **No metric invention, no severity reclassification.** See the Cardinal Rule.

## Where it sits in the pipeline

```
ga4-website-maa   ->  {client}/MAAs-GA4/{date}.md   (the analysis, pages 2+)
                                   |
                                   v   (final step, every run)
ga4-client-view   ->  derives the page1-spec JSON from that MAA
                  ->  renders page1 HTML + PDF, and (optional) the merged full PDF
```

It is the **last** step and runs **every** time, adding presentation only. This is
the local "display version" for now; it is written to be lifted into the Managed
Agent later as the render stage (see "Cloud transition" at the end).

---

## THE CARDINAL RULE: derive, don't invent

Every value on page 1 must trace to a specific line in the source MAA. If you
cannot point to where the MAA says it, **leave it out.**

Concrete prohibitions:
- **Never add a metric** not present in the MAA.
- **Never upgrade a hedge into confidence** (if the MAA says "still gathering
  data," the pill/flag may not read as proven).
- **Never invent reassurance** ("tracking is solid", "everything's healthy").
- **Never editorialize a chart.** Title it with the plain metric name only.
- **Never reclassify severity.** The MAA's internal Clarity grade and the flags
  drive the stoplight; you translate, you do not re-decide.

The machinery still stays internal (per `ga4-website-maa` report-format): no
"Opaque/Hazy/Clear," no "attribution coverage," no thread names on the page.

---

## The display standard (what every page 1 must have)

Full detail in `references/style-spec.md`. The non-negotiables:

1. **Stoplight color, always.** Red = critical or **missing/broken tracking** (a $0
   where money should be, an untracked form, a dropped event). Yellow = watch /
   needs confirmation. Green = healthy / producing. Grey = flat only. Applied to
   the KPI pills, the "what stands out" flags, and the action items. If a sentence
   says "isn't tracking / missing / 0 / dropped / broken," its marker is red.
2. **Prepared by** the human owner (name, role, and photo when available).
3. **Client logo** in the header (falls back to the styled client name).
4. **Two standard charts**, identical across clients: 13-week primary-conversion
   **trend** (left) and **channel breakdown** bars (right).
5. **"What to do next" is an owned checklist** — every item names who does it
   (`start` = the one first move, `us` = we're on this, or a named client owner).
6. **Source-citations footer**: project link, live GA4 link, report date, prepared-by.
7. **"We" voice** everywhere ("we're getting your calls tracked," not "your calls
   aren't tracked"). No em dashes.

---

## Step 1 — Load the source MAA (read-only)

Read the latest dated MAA for the client (the markdown `ga4-website-maa` produced).
That MAA is the **only** source of numbers, findings, and actions. If no dated MAA
exists for the period, stop — build the MAA first, there is nothing to render.

## Step 2 — Derive the page1-spec JSON

Project the MAA into the `page1-spec` JSON contract in
`references/page1-schema.md`. That file has the field-by-field mapping (which MAA
section feeds each field) and the stoplight-state rules. In short:

| page-1 element | Derived from the MAA |
|---|---|
| `pulse` | The MAA's business-pulse / bottom-line paragraph, in "we" voice, confident (no trailing caveat). |
| `pills` (4) | The headline number + its two-three companions in the MAA's own words (leads / calls / forms / lead rate, or the model's equivalents). Each carries a comparison + a `state`. |
| `left_chart` (trend) | The MAA's 13-week primary-conversion trend series. `last_state:"bad"` only if the MAA reads the latest move as a real decline. |
| `right_chart` (bars) | The MAA's channel (or model-lens) Winners table, top rows. |
| `callout` (optional) | A single data-trust story the MAA foregrounds (e.g. off-domain checkout, spam-in-Direct). Omit if the MAA has none. |
| `flags` | The MAA's "what it means" points, each routed to a stoplight state. |
| `todo` | The MAA's "what to do next" actions, each tagged `start` / `us` / client-owner from the MAA's inline owner tags. |
| `citations` | Project URL, GA4 property URL, report date. |

Do not invent a fourth pill or a flag the MAA doesn't support. Fewer real items
beat padded ones.

## Step 3 — Attach assets (with clean fallbacks)

Optional, per client — the render degrades gracefully if any are missing:
- `logo`: path to the client logo (embedded as a data URI). Missing → styled name only.
- `prepared_by`: `{name, role, photo}`. Missing photo → name/role text only.
- `citations.project_url` / `ga4_url`: the client's Basecamp project and GA4 property.

The client→owner map and logo files live with the client config; if none is on
hand, render without them and note the gap rather than blocking.

## Step 4 — Render

```
python3 scripts/render_page1.py  <page1-spec.json>  <out.html>     # HTML (self-contained)
python3 scripts/render_pdf.py    <out.html>         <out.pdf>      # print-ready PDF (Chromium)
python3 scripts/merge_full.py    <page1.pdf> <maa.pdf> <full.pdf>  # optional: page1 + MAA
```

`render_page1.py` draws the charts as inline SVG (dataviz rules) and applies the
stoplight system from the JSON `state` fields — never hand-build the SVG or the
HTML. `render_pdf.py` uses the pre-installed Chromium (do NOT run
`playwright install`). `merge_full.py` needs `pypdf`; the MAA PDF is produced
separately (export the agent's markdown, or the Google Doc, to PDF).

## Step 5 — Verify before presenting (required)

1. **Trace-check:** every pill, flag, chart point, and action maps to a line in the
   MAA. Anything that doesn't is removed, not "supported later."
2. **Stoplight correctness:** every critical/missing item is red; nothing green
   overclaims a hedge.
3. **Voice:** "we," not "you/they," throughout; **zero em dashes**.
4. **Sums:** pill sub-counts and chart rows agree with the MAA's headline (e.g. a
   leads pill that says "163 line + 103 clicks" must match the MAA's call split).
5. **Chart endpoints** equal the MAA's latest numbers.

This is a **draft for review**, same as the MAA. Never auto-publish.

## Cloud transition (later)

This skill is deliberately self-contained (SKILL.md + `scripts/` + `references/`)
so it can be lifted into the `ga4-website-maa` Managed Agent as the render stage —
the seam is the "Rendering" note in that skill's `report-format.md`. To promote it:
attach this skill to the agent (or fold `render_page1.py` in as a bundled script),
and have the agent run Step 2's derivation on the MAA it just wrote, then the Step
4 scripts. Keep the Cardinal Rule intact: the agent derives page 1 from its own
report, it does not re-analyze.

## Connected

- `ga4-website-maa` — produces the MAA this stage renders.
- `references/style-spec.md` — the display standard.
- `references/page1-schema.md` — the JSON contract + derivation mapping.
- `references/example-fairmount.json` — a worked page1-spec.
- `scripts/render_page1.py`, `scripts/render_pdf.py`, `scripts/merge_full.py`.
